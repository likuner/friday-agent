...
export default function AuthPage({ initialMode = 'login' }: AuthPageProps) {
  ...
  return (
    <main className="relative min-h-screen bg-canvas px-5 py-10 sm:grid sm:place-items-center">
        ...
        <div className="p-7 sm:p-12">
          ...
          <Form layout="vertical" onFinish={submit} requiredMark={false} className="mt-5">
            <Form.Item name="username" label="用户名" rules={[{ required: true, message: '请输入用户名' }]}>
              <Input prefix={<UserOutlined />} size="large" placeholder="3-32 位用户名" />
            </Form.Item>
            <Form.Item name="password" label="密码" rules={[{ required: true, min: 6, message: '密码至少 6 位' }]}>
              <Input.Password prefix={<LockOutlined />} size="large" placeholder="至少 6 位密码" />
            </Form.Item>
            <Form.Item label="验证码" required>
              <div className="flex gap-2">
                <Form.Item name="captcha" noStyle rules={[{ required: true, message: '请输入验证码' }]}>
                  <Input size="large" placeholder="输入图片中的字符" />
                </Form.Item>
                <button
                  type="button"
                  onClick={refresh}
                  className="h-10 w-[120px] shrink-0 overflow-hidden rounded-lg border border-line-strong bg-soft"
                  title="刷新验证码"
                >
                  {code.image ? <img src={code.image} alt="验证码" className="h-full w-full object-cover" /> : <ReloadOutlined />}
                </button>
              </div>
            </Form.Item>
            <Button
              ...
              block
              loading={loading}
              className="mt-1 h-11 rounded-xl border-0 bg-gradient-to-r from-[#4d6bfe] to-[#6a86ff] shadow-[0_10px_24px_-12px_rgba(77,107,254,.9)] transition-transform hover:scale-[1.01]"
            >
