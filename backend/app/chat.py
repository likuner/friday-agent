...
async def chat(
    ...
    logger.info(
        "节点[接收消息] conversation=%s user=%s replay=%s条(≈%s tok) overflow=%s条(≈%s tok) 摘要=%s content=%r deep_thinking=%s web_search=%s",
        conversation.id, user.username,
        len(window.replay), window.replay_tokens, len(window.overflow), window.overflow_tokens,
        bool(conversation.summary), payload.content[:100], payload.deep_thinking, payload.web_search,
    )
