...
| Agent 内置工具 | 侧边栏「工作区」组标题旁的 **+** 选定本机目录（系统原生目录选择框，DeepSeek Harness 式，选定后不可更换），会话列表按「工作区 / 对话」分组；工作区会话自动开启 Agent 工具：Bash / 文件读写 / 任务规划四件套，写入限制在工作区与 deny 封禁敏感路径。权限模式随会话记住（chip 下拉选择：每步确认 / 工作区免确认 / 只读 / 自动，默认每步确认，确认卡片 120s 超时自动拒绝） |
| 多模态图片 | 一次最多上传 9 张图片，转 base64 作为多模态输入送模型，消息中固定尺寸裁剪展示、点击可预览 |
| 会话管理 | 历史记录列表、服务端搜索（按标题）、重命名、删除；列表分页每页 20 条，滚到底自动加载下一页（按钮兜底） |
...
| `GET` | `/api/conversations` | 会话分页列表：`q` 标题关键词、`kind`=all/workspace/plain、`limit`(默认 20，≤100)、`offset`；返回 `{items,total,has_more}` |
